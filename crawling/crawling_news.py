...
for search_keyword in search_keywords:
    driver.get(f'https://search.naver.com/search.naver?where=news&query={search_keyword}&pd=2')
    ...
    for title, content in zip(titles, contents):
        if str(content.text).find(search_keyword) != -1:
            ...
            data.append(item)
    with open(f'news_crawling_{search_keyword}.json', 'w', encoding='utf-8') as f :
